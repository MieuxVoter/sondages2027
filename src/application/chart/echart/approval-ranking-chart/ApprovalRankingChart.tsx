...
import { approvalRankingChartConfig } from './rankingChartConfig';
import { useCandidateApprovalRankingSeries } from './useCandidateApprovalRankingSeries';

interface ApprovalRankingChartProps {
  isThumbnail?: boolean;
}

export const ApprovalRankingChart: React.FC<ApprovalRankingChartProps> = ({ isThumbnail = false }) => {
  const [selectedCandidates, setSelectedCandidates] = useState<Set<string>>(new Set());

  const candidateRankingsSeries = useCandidateApprovalRankingSeries(selectedCandidates, isThumbnail);
  ...
  const rankingChartOption: EChartsOption = {
    ...approvalRankingChartConfig,
    legend: { show: false },
    yAxis: { ...approvalRankingChartConfig.yAxis, max: candidateRankingsSeries.length },
    series
  }
      ...
      <BorderLayout
        north={
          <>
            {!isThumbnail &&
              <ChartTitle
                title="Classement des candidats à l'éléction présidentiel 2027 - Vote par approbation"
