...
for trace in plotly_data["data"]:
    ...
    series_data.append({"name": name, "color": color, "data": x_data, "y_labels": y_labels})
...
echarts_config = {
    ...
    "legend": {"data": [s["name"] for s in series_data], "bottom": 0, "orient": "horizontal"},
    ...
    "yAxis": {
        "type": "category",
        "data": [
            candidate.replace("<b>", "").replace("</b>", "").split("<br>")[0].strip()
...
for series in series_data:
    echarts_series = {
        ...
        "stack": "satisfaction",
        "data": series["data"],
        "itemStyle": {"color": series["color"].replace("rgb(", "rgba(").replace(")", ", 1)")},
        "barWidth": "60%",
    }
